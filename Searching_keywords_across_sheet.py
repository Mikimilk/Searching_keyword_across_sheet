...
class Searching():
    def __init__(self):
        #self.draw = Draw()
        #self.conf_file = self.draw.input_file
        #self.out_path = self.draw.out_file
        self.conf_file = input()
        self.out_path = input()
        self.df = pd.ExcelFile(self.conf_file) 

        colsname = ['server_name' ,'header_name' ,'p_name']

        com_sys = pd.read_excel(self.df, 'COMPUTERSYSTEM',usecols='G:I',header=None ,names=self.colsname ,skiprows=4)
        app = pd.read_excel(self.df, 'APPLICATION' ,usecols='F:CN' ,skiprows=4)
        ...
        #Export to .xlsx
        export_excel = conf_frame.to_excel(self.out_path, index = None, header=True)
